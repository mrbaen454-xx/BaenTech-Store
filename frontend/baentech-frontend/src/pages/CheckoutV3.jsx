...
const paymentMethods = [
  {
    value: "QRIS",
    label: "QRIS",
    desc: "Invoice Xendit dengan pembayaran QRIS.",
  },
  {
    value: "BANK_TRANSFER",
    label: "Bank Transfer",
    desc: "Transfer bank melalui invoice Xendit.",
  },
  {
    value: "E_WALLET",
    label: "E-Wallet",
    desc: "Dompet digital melalui invoice Xendit.",
  },
  {
    value: "CREDIT_CARD",
    label: "Credit Card",
    desc: "Kartu kredit melalui invoice Xendit.",
  },
];
...
function CheckoutV3() {
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      totalProducts: Number(checkoutData.totalProducts || 0) || items.length,
      totalPrice:
  ...
  const loadCheckoutPageData = async () => {
    try {
      ...
      setCheckoutData({ items: [], totalItems: 0, totalProducts: 0, totalPrice: 0 });
      setAddresses([]);
      setError(message);
  ...
  const validateForm = () => {
    ...
    if (!mainAddress.recipientName) return "Nama penerima belum lengkap.";
    if (!mainAddress.phoneNumber) return "Nomor HP belum lengkap.";
    if (!mainAddress.fullAddress) return "Alamat lengkap belum lengkap.";
    if (!mainAddress.city) return "Kota belum lengkap.";
    if (!mainAddress.province) return "Provinsi belum lengkap.";
    if (!mainAddress.postalCode) return "Kode pos belum lengkap.";
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      createdOrder = await checkoutApi(checkoutPayload);
      ...
      showToast({
        type: "info",
        message: "Order berhasil dibuat. Sedang membuka invoice Xendit...",
      });

      const payment = await createPaymentWithRetry({ orderId, paymentMethod });
      ...
      showToast({
        type: "success",
        message: "Invoice Xendit berhasil dibuat. Mengarahkan ke pembayaran...",
      });
      ...
      if (orderId) {
        ...
        const message =
          "Order berhasil dibuat, tetapi invoice Xendit belum terbuka. Silakan klik Bayar Sekarang di halaman pembayaran.";

        setError(message);
        showToast({ type: "warning", message });

        setTimeout(() => {
          navigate("/payment/finish", { replace: true });
        }, 700);
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-950 dark:bg-slate-950">
      ...
      <main className="mx-auto max-w-7xl px-3 py-5 sm:px-6 lg:px-8 lg:py-10">
        <div className="mb-6 flex flex-col gap-4 sm:mb-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            <Link
              to="/cart"
              className="mb-4 inline-flex items-center gap-2 rounded-2xl bg-white px-4 py-2.5 text-xs font-black text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:text-blue-600 active:scale-[0.98] dark:bg-slate-900 dark:text-slate-200 sm:text-sm"
            >
              <ArrowLeft size={17} />
            ...
            <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-500 dark:text-slate-400 sm:text-base">
              Checkout akan membuat order lalu langsung membuka invoice Xendit.
            </p>
          </div>
      ...
      {!address ? (
        <div className="rounded-3xl border border-dashed border-yellow-300 bg-yellow-50 p-5 dark:border-yellow-900/60 dark:bg-yellow-950/20">
          ...
          <p className="mt-2 text-sm font-semibold leading-6 text-yellow-700/80 dark:text-yellow-300/80">
            Tambahkan alamat di halaman profile dan jadikan sebagai alamat utama sebelum checkout.
          </p>
          <Link
            to="/profile"
  ...
  return (
    <div className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400 sm:text-sm">
            Produk berikut akan diproses menjadi order.
          </p>
        </div>
      ...
      <div className="space-y-3">
        {items.map((item) => {
          const itemId = item.id || item.cartItemId || item.itemId || item.productId;
          ...
          return (
            ...
            >
              <div className="h-16 w-16 overflow-hidden rounded-2xl bg-white dark:bg-slate-900 sm:h-[76px] sm:w-[76px]">
                <img
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">
                  Qty {Number(item.quantity || 0)} × {formatRupiah(getItemPrice(item))}
                </p>
              </div>
    ...
    <div className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
      <div className="mb-5 flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400 sm:text-sm">
            Sistem akan membuat invoice Xendit sesuai metode yang dipilih.
          </p>
        </div>
      </div>
        ...
        {paymentMethods.map((method) => {
          ...
          return (
            <button
              ...
              className={`min-w-0 rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 active:scale-[0.98] ${
                active
            ...
            >
              ...
              <p className="mt-1 text-xs font-semibold opacity-70">{method.desc}</p>
            </button>
          );
        })}
      </div>
    ...
    <aside className="min-w-0 lg:sticky lg:top-28 lg:self-start">
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 sm:text-sm">
              Setelah klik bayar, kamu langsung diarahkan ke Xendit.
            </p>
          </div>
        </div>

        <div className="mt-6 space-y-4">
          <SummaryRow label="Produk Dipilih" value={`${summary.totalProducts} produk`} />
          <SummaryRow label="Total Item" value={`${summary.totalItems} item`} />
          <SummaryRow label="Subtotal" value={formatRupiah(summary.totalPrice)} />
        ...
        <button
          ...
          className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-3.5 text-sm font-black text-white shadow-lg shadow-blue-500/30 transition hover:-translate-y-0.5 hover:bg-blue-700 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
        >
          {submitting ? (
          ...
          {submitting ? "Membuka Xendit..." : "Bayar Sekarang"}
        </button>
        ...
        <p className="mt-4 rounded-2xl bg-slate-50 px-4 py-3 text-xs font-semibold leading-5 text-slate-500 dark:bg-slate-950 dark:text-slate-400">
          Order dibuat otomatis, payment dibuat di Xendit, lalu status akan berubah dari callback Xendit.
        </p>
      </div>
    </aside>
  );
  ...
  return (
    <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-900 sm:p-12">
      ...
      <p className="mx-auto mt-2 max-w-lg text-sm font-semibold leading-6 text-slate-500 dark:text-slate-400">
        Pilih produk dari halaman keranjang terlebih dahulu, lalu klik Lanjut Checkout.
      </p>
      <Link
        to="/cart"
...
async function createPaymentWithRetry(payload) {
  const retryDelays = [0, 900, 1600];
  let lastError = null;
  ...
  for (let index = 0; index < retryDelays.length; index += 1) {
    ...
    try {
      return await createXenditPaymentApi(payload);
    } catch (err) {
      lastError = err;
      console.log(`CREATE XENDIT PAYMENT ATTEMPT ${index + 1} FAILED:`, err);
    }
  }
...
function savePendingPayment({ order, payment, paymentMethod, redirectUrl }) {
...
function getPaymentRedirectUrl(payment) {
